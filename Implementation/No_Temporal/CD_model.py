...
def preprocess_graph(graph: HeteroData):
    ...
    normalized_adj = preprocess_adj(torch_user_adj, num_users) #keep this in mind
...
class ModCDModel(nn.Module):
    ...
    def forward(self, graph: HeteroData):
        ...
        self.reconstruct, self.clusters_distance, self.node_emb = self.cd_encoder_decoder(
            user_feature.to(self.device),
        ...
        if not self.pretrain:
            ...
            new_edge_index = new_edge_index[:, torch.randperm(new_edge_index.size(1))[:new_edge_num]]

            k_partition = clusters(
                self.node_emb.detach().to('cpu'),
                edge_index.to('cpu'),
                num_communities,
                self.cluster_choice,
                self.ensure_comm_num
                )
...
class CDEncoderDecoder(nn.Module):
    def __init__(self, hidden_dim, output_dim, args):
        ...
        self.weight_layer1 = torch.nn.Parameter(init_weight(args.embedding_dim, hidden_dim))
    ...
    # user_feature is already X_shared
