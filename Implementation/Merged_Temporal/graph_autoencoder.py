...
def build_sparse_extended_adjacency(
    ...
    """
    Sparse Eq. (9):  M = (B + B² + … + Bᵗ) / t

    Uses torch.sparse_coo_tensor matrix multiplication to stay O(E × t)
    instead of O(N² × t).  For very sparse graphs (E << N²) this is
    orders of magnitude cheaper.

    Returns
    -------
    M_ei  : (2, E_M)  edge indices of non-zero M entries
    M_w   : (E_M,)    corresponding M values (float32)
    """
...
def sample_negative_edges(
    ...
    """
    Sample `num_neg` random negative edges (pairs not in pos_edge_index).
    Uses rejection sampling; fast when E << N².
    """
