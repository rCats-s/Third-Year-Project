"""
prepare_model_ready_graph.py  (optimised for 10k users)
========================================================

Key changes vs the original
----------------------------
1. Single JSON pass  – tweet_0.json is loaded ONCE and yields both
   timestamps and tweet texts in the same loop.  The original loaded
   it twice (rebuild_timestamps + load_user_tweets), which doubles
   I/O time and peak RAM for large files.

2. Batched RoBERTa for descriptions  – the original ran one forward
   pass per user (10k individual CUDA calls for 10k users).  The
   batch version groups users into batches of DESC_BATCH_SIZE (64)
   and runs ~156 calls instead of 10,000.  Expected speedup: ~50x.

3. Batched RoBERTa for tweets  – the original ran one forward pass
   per tweet (50k calls for 10k users × 5 tweets).  The batch version
   flattens all (user, slot) pairs into a single list, tokenises them
   in bulk, and runs ~780 batched calls.  Expected speedup: ~50x.

4. GPU cache cleared between desc and tweet passes to avoid OOM on
   low-VRAM cards (MX450 has 2 GB).

5. tqdm progress bars on all long loops.

6. Memory estimate printed before the large tweet_emb allocation so
   the user knows what is coming.

Memory budget for 10k users
----------------------------
  tweet_emb  (float32): 10k × 5 × 8 × 768 × 4 B  ≈  1.17 GB  (RAM, CPU)
  desc_emb   (float32): 10k × 768 × 4 B            ≈  30 MB
  RoBERTa model        (GPU)                        ≈  500 MB
  Peak GPU per desc batch (64):  64 × 64 × 768 × 4 ≈  12 MB    (fine)
  Peak GPU per tweet batch (64): 64 × 8  × 768 × 4 ≈  1.6 MB   (fine)
  Total RAM peak                                    ≈  2.5 GB   (safe on 8 GB)
"""
...
# ═══════════════════════════════════════════════════════════════════════════
#  Stage 1 – single JSON pass (tweets + timestamps)
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
#  Stage 2 – temporal features (unchanged logic, already fast)
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
#  Stage 3a – batched description embeddings
# ═══════════════════════════════════════════════════════════════════════════
...
def build_desc_emb_batched(
    ...
    """
    Batch all descriptions and run RoBERTa in groups of `batch_size`.

    Original: 1 forward pass per user  → 10,000 calls for 10k users
    This:     1 forward pass per batch → ~156 calls for 10k users (64/batch)
    """
    N        = len(user_ids)
...
# ═══════════════════════════════════════════════════════════════════════════
#  Stage 3b – batched tweet embeddings
# ═══════════════════════════════════════════════════════════════════════════
...
def build_tweet_emb_batched(
    ...
    """
    Flatten all (user_idx, tweet_slot, text) into one list, tokenise in bulk,
    run RoBERTa in batches, then scatter results back into the output tensor.

    Original: 1 forward pass per tweet → 50,000 calls for 10k users × 5 tweets
    This:     1 forward pass per batch →  ~780 calls (64/batch)
    """
    N = len(user_ids)
...
# ═══════════════════════════════════════════════════════════════════════════
#  Stage 4 – load user descriptions (still one-shot, file is small)
# ═══════════════════════════════════════════════════════════════════════════
...
def main():
    ...
    users_with_ts = sum(1 for xs in tweet_timestamps.values() if xs)
    users_with_tw = sum(1 for xs in tweet_texts.values() if xs)
    print(f"  users with timestamps : {users_with_ts:,} / {N:,}")
    print(f"  users with tweet texts: {users_with_tw:,} / {N:,}")

    # ── Stage 2: temporal features ────────────────────────────────────────
    print("\n[Stage 2] Computing temporal features …")
    temporal_num, temporal_cat = compute_temporal_features(tweet_timestamps, user_ids)

    num = torch.tensor(
        np.concatenate([user_num_feat, temporal_num], axis=1),
        dtype=torch.float32,
    )
    cat = torch.tensor(
        np.concatenate([user_cat_feat, temporal_cat], axis=1),
        dtype=torch.float32,
    )
    ...
    print("  mean  :", round(tweet_emb.mean().item(), 4))
    print("  std   :", round(tweet_emb.std().item(),  4))
    users_with_tw_emb = (tweet_len.sum(dim=1) > 0).sum().item()
    print(f"  users with tweets: {users_with_tw_emb} / {N}")
    nz_tok = (tweet_emb.abs().sum(dim=-1) > 0).sum().item()
    total_tok = N * MAX_TWEETS_PER_USER * MAX_WORDS_PER_TWEET
    print(f"  nonzero token embeddings: {nz_tok} / {total_tok}")
