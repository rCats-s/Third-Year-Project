...
hybrid_model.py
===============
Integrates BotDCGC (graph autoencoder + clustering) with CACL
(community-aware contrastive learning on a heterogeneous graph).

Training pipeline
-----------------
Phase C  –  Shared encoder  →  BotDCGC  →  pseudo-label extraction
Phase D  –  Inject X_shared into the HeteroData graph  →  CACL forward pass
Phase E  –  Attach pseudo-label classification loss head to CACL
Phase F  –  Train CACL with:
              • CACL contrastive loss  (unsupervised_cl_loss)
              • Pseudo-label classification loss  (cross-entropy)
            BotDCGC encoder weights are *frozen* during Phase F.
"""
...
class TrainCfg:
    ...
    # Phase C – BotDCGC joint (reconstruction + contrastive + cluster)
    ...
    cacl_lr:           float = 1e-3
    cacl_wd:           float = 1e-4
    # Pseudo-label confidence threshold (only use confident BotDCGC predictions)
    pseudo_conf_thr:   float = 0.80
    log_every:         int   = 10


# ═══════════════════════════════════════════════════════════════════════════
#  Simple args shim so HeteroGraphConvModel is satisfied
# ═══════════════════════════════════════════════════════════════════════════
...
class HybridModel(nn.Module):
    """
    Shared-encoder hybrid of BotDCGC and CACL.

    Submodule layout
    ----------------
    self.user_encoder   – Section 4.1  (shared by both branches)
    self.gae            – Section 4.2  (BotDCGC graph autoencoder)
    self.clustering     – Section 4.3  (student-t cluster assignment)
    self.cacl_model     – CACL HGcnCLModel  (hetero encoder + projector + classifier)
    """
    ...
    def __init__(
        ...
        # ── CACL HeteroGraphConvModel (encoder) ───────────────────────────
        # in_channels for HeteroGraphConvModel:
        #   'user' dim is overridden to embed_dim (X_shared is injected)
        ...
        # output dim of HeteroGraphConvModel = out_channels + embed_dim (cat of conv + raw)
    ...
    # ═══════════════════════════════════════════════════════════════════════
    #  Phase C helpers: BotDCGC forward + pseudo-label extraction
    # ═══════════════════════════════════════════════════════════════════════
    ...
    def extract_pseudo_labels(
        ...
        """
        Phase C output: hard cluster assignments and a confidence mask.

        Parameters
        ----------
        conf_threshold : float
            Only nodes whose max Q value exceeds this threshold are included
            in the mask; the rest are treated as unlabelled during Phase F.

        Returns
        -------
        pseudo_labels : (N,) long tensor
        pseudo_mask   : (N,) bool tensor  (True = confident / use this node)
        """
        self.eval()
    ...
    # ═══════════════════════════════════════════════════════════════════════
    #  Phase D helper: inject X_shared into HeteroData graph
    # ═══════════════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════════════
    #  Phase E / F: CACL forward with pseudo-label loss
    # ═══════════════════════════════════════════════════════════════════════
    ...
    def cacl_forward_with_pseudo(
        ...
        """
        Phase D + E + F combined forward:
        1. Run frozen user_encoder -> X_shared
        2. Augment original graph twice
        3. Inject X_shared into both augmented views
        4. Run CACL on both views
        5. Compute contrastive loss
        6. Compute pseudo-label classification loss
        """
...
def train_hybrid(
    ...
    """
    End-to-end training of the hybrid model.

    Phase C  –  Pre-train BotDCGC (reconstruction only, then joint).
    Phase C' –  Initialise cluster centres with K-means.
    Phase C'' –  Extract pseudo-labels.
    Phase F  –  Fine-tune CACL (contrastive + pseudo-label loss).
    """
    cfg = cfg or TrainCfg()
    model.to(device)
    ...
    # ──────────────────────────────────────────────────────────────────────
    # Phase C-2: K-means initialise cluster centres
    # ──────────────────────────────────────────────────────────────────────
    ...
    # ──────────────────────────────────────────────────────────────────────
    # Phase C-3: Joint BotDCGC training (recon + contrast + cluster)
    # ──────────────────────────────────────────────────────────────────────
    ...
    # ──────────────────────────────────────────────────────────────────────
    # Phase C-4: Extract pseudo-labels
    # ──────────────────────────────────────────────────────────────────────
    ...
    # ──────────────────────────────────────────────────────────────────────
    # Phase F: CACL fine-tuning (BotDCGC encoder frozen)
    # ──────────────────────────────────────────────────────────────────────
