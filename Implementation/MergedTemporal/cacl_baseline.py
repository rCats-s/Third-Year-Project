...
cacl_baseline.py
================
CACL-only baseline for fair comparison against the hybrid model.

Architecture
------------
UserFeatureEncoder  (Section 4.1, identical to hybrid_model.py)
        │
        ▼   X_shared injected into HeteroData graph['user'].x
HeteroGraphConvModel  (heterogeneous GNN encoder)
        │
        ├──► MLPProjector  →  unsupervised contrastive loss  (always)
        └──► Classifier    →  cross-entropy on TRUE labels   (optional)

What is deliberately absent
----------------------------
• BotDCGCGraphAutoEncoder  (gae)
• ClusteringModule
• Pseudo-label extraction / confidence masking
• Hungarian-matching evaluation
• Any Phase-C pre-training

Loss function comparison
------------------------
                    CACLBaseline            HybridModel (Phase F)
──────────────────────────────────────────────────────────────────
contrastive         unsupervised_cl_loss    unsupervised_cl_loss
classification      cross_entropy(true_lbl) cross_entropy(pseudo_lbl)
source of labels    ground truth split      BotDCGC cluster assignments
BotDCGC encoder     absent                  frozen (no grad)
──────────────────────────────────────────────────────────────────

Config dataclasses
------------------
FeatureEncoderCfg  –  identical to hybrid_model.py  (shared import-safe)
CACLCfg            –  identical fields to hybrid_model.py
                       cl_beta here weights TRUE-label loss,
                       in hybrid it weights pseudo-label loss
BaselineTrainCfg   –  single-phase training knobs (no Phase-C fields)
"""
...
# ═══════════════════════════════════════════════════════════════════════════
#  Config dataclasses  (field-for-field identical to hybrid_model.py so
#  the same config object can be passed to both models in an experiment)
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
#  Internal args shim  (HeteroGraphConvModel.__init__ expects an object
#  with .num_layer, .momentum, .dropout rather than a dataclass)
# ═══════════════════════════════════════════════════════════════════════════
...
class CACLBaseline(nn.Module):
    """
    CACL-only baseline bot-detection model.

    Submodule layout
    ----------------
    self.user_encoder  –  UserFeatureEncoder  (Section 4.1)
    self.cacl_model    –  HGcnCLModel
                            ├── encoder    HeteroGraphConvModel
                            ├── projector  MLPProjector
                            └── classifier Classifier  (None if use_classifier=False)

    Parameters
    ----------
    metadata       : (node_types, edge_types) from a HeteroData graph
    in_channels    : dict mapping each node type to its raw feature dim.
                     The 'user' entry is overridden internally to feat_cfg.embed_dim
                     because X_shared replaces graph['user'].x at runtime.
    feat_cfg       : FeatureEncoderCfg
    cacl_cfg       : CACLCfg
    use_classifier : Whether to attach and train the MLP classification head.
                     Set True when ground-truth train labels are available.
    """
    ...
    # ── utility: inject X_shared into the HeteroData graph ───────────────
    ...
    # ── forward for a single graph view ──────────────────────────────────
    ...
    def forward(
        ...
        """
        Single-view forward pass (used during inference / evaluation).

        Returns
        -------
        dict with keys:
            X_shared  – (N, D)     shared user embeddings
            proj      – (N, proj_out) per-node projections (user type)
            pred      – (N, 2)     class probabilities (None if no classifier)
        """
    ...
    def compute_losses(
        ...
        """
        Compute the two CACL losses from a two-view forward dict.

        Loss-1  L_contrastive
        ---------------------
        unsupervised_cl_loss operates only on split==0 nodes
        (training nodes see both views; val/test nodes are excluded).

        Loss-2  L_classification  (only if use_classifier=True and labels given)
        -------------------------
        Standard cross-entropy on training nodes (split==0) using
        *ground-truth* labels. This replaces the pseudo-label loss in
        the hybrid model; the weight coefficient cl_beta is shared so
        the two models can be compared under identical hyper-parameters.

        Total loss
        ----------
        L = cl_alpha * L_contrastive + cl_beta * L_classification

        Parameters
        ----------
        fwd    : output of forward_two_views()
        split  : (N,) long tensor  0=train, 1=val, 2=test
        labels : (N,) long tensor of ground-truth class labels  (0=human, 1=bot)
                 Required when use_classifier=True; ignored otherwise.
        """
        proj1 = fwd["proj1"]
        proj2 = fwd["proj2"]
        pred  = fwd["pred"]
    ...
    def predict(
        ...
        """
        Returns hard predictions (0=human, 1=bot) of shape (N,).
        Uses the classifier head if present, otherwise falls back to the
        nearest-centroid heuristic on proj embeddings.
        """
        self.eval()
...
def train_cacl_baseline(
    ...
    """
    Single-phase CACL baseline training.

    Unlike train_hybrid(), there is no BotDCGC pre-training phase.
    All parameters (user_encoder + cacl_model) are trained jointly from
    the first epoch under the combined contrastive + classification loss.

    Parameters
    ----------
    split  : Integer node-split tensor (0=train, 1=val, 2=test).
             unsupervised_cl_loss internally filters to split==0.
             Classification loss also operates on split==0.
    labels : True labels.  Pass None to train with contrastive loss only
             (pure self-supervised mode).

    Returns
    -------
    dict with keys 'history' and 'final'.
    """
