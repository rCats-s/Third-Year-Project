...
# ---------------------------------------------------------------------------
# 4.1.1  Numerical feature encoder
# ---------------------------------------------------------------------------
class NumericalEncoder(nn.Module):
    ...
    IMPORTANT:
    The input 'num' may contain both:
    - static profile/activity numerical features
    - temporal numerical features derived from tweet timestamps

    Examples of temporal numerical features:
    - mean inter-tweet gap
    - std inter-tweet gap
    - tweets per active day
    - posting-hour entropy
    - weekend posting ratio
    - night posting ratio

    Normalisation is handled outside (in the dataset).
    """
...
# ---------------------------------------------------------------------------
# 4.1.2  Categorical feature encoder
# ---------------------------------------------------------------------------
class CategoricalEncoder(nn.Module):
    ...
    IMPORTANT:
    The input 'cat' may contain both:
    - static categorical / boolean account features
    - temporal behavioural flags derived from timestamps

    Examples of temporal categorical features:
    - night-heavy account
    - weekend-heavy account
    - highly regular posting
    - bursty posting
    """
...
# ---------------------------------------------------------------------------
# 4.1.4  Tweet text encoder
# ---------------------------------------------------------------------------
class TweetEncoder(nn.Module):
    """
    Encodes tweet TEXT only.

    For each tweet the words are pre-encoded with RoBERTa.
    An LSTM reads the word sequence; hidden states are averaged
    per tweet, then tweet representations are averaged per user.

    Input:
        tweet_emb : (B, M, max_words, roberta_dim)
        tweet_len : (B, M)  -> actual word count per tweet

    NOTE:
    tweet_len is token length, not temporal length.
    Tweet chronology across time is NOT modelled here.
    Temporal behaviour should therefore be injected upstream through
    'num' and 'cat' unless a separate temporal encoder is added later.
    """
...
# ---------------------------------------------------------------------------
# 4.1.5  Feature fusion with Bi-LSTM
# ---------------------------------------------------------------------------
class FeatureFusion(nn.Module):
    """
    Stacks the four sub-embeddings into a 4-step sequence:
        [numerical, categorical, description, tweet]

    Then applies a Bi-LSTM and mean pooling to produce the final
    shared node embedding x_i in R^D.

    If temporal features are included in num/cat, they are fused here
    together with all other feature types automatically.
    """
